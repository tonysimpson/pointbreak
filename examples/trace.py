...
# XXX the symbols interface and its interaction with breakpoints and traps 
# are WIP
class Tracer:
    def __init__(self):
        self.depth = 0
    ...
    def trace(self, db):
        ...
        depth = self.depth
        def trace_exit(db):
            if self.depth == depth: # because of recursion we can have more than one bp on the return address
                self.depth -= 1
                return False
            return True
